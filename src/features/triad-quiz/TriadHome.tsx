...
import { TriadTutorial } from './TriadTutorial';
import { ModuleStats } from '../../components/platform/ModuleStats';

interface TriadHomeProps {
  onStartQuiz: (isChallenge: boolean, targetLevel: number) => void;
  onHome: () => void;
}

export function TriadHome({ onStartQuiz, onHome }: TriadHomeProps) {
  ...
  const unlockInfo = [
    { label: '基礎 C 根音', level: 1 },
    { label: 'C, D, G 根音', level: 2 },
    { label: 'C 大調順階和弦', level: 4 },
    { label: '隨機 12 半音根音', level: 6 },
    { label: '全隨機 (打亂音符)', level: 9 },
  ];
